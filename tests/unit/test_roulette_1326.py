...
def test_roulette_1326(monkeypatch, mock_bet, mock_table, mock_game):
    ...
    player = casino.players.Roulette1326(table)
    ...
    assert isinstance(player.state, casino.players.Roulette1326NoWins)

    player.place_bets()
    player.win(table.bets[0])
    assert player.stake == 101
    assert isinstance(player.state, casino.players.Roulette1326OneWin)

    player.place_bets()
    player.win(table.bets[1])
    assert player.stake == 104
    assert isinstance(player.state, casino.players.Roulette1326TwoWins)

    player.place_bets()
    player.win(table.bets[2])
    assert player.stake == 106
    assert isinstance(player.state, casino.players.Roulette1326ThreeWins)
